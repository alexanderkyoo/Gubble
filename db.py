...
def insertItem(item_info):
    with sqlalchemy.orm.Session(_engine) as session:
        ...
        messages=[
            ...
            {"role": "user", "content": "How many days would you predict that it would take someone to run out of the following." + item_info.get('item_name') + "Give an exact number of days, and only a single number. Format like this [number]"}
        ]
        )
